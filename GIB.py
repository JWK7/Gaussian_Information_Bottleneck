...
def direct_A(kwargs):
    return np.matmul(kwargs["cov_XgY"], np.linalg.inv(kwargs["cov_X"]))
...
def get_init_A(full_size,compressed_size):
    return np.random.uniform(-1,1,(compressed_size,full_size))

def get_init_Cov_Epsil(A_shape):
    return np.cov(np.random.normal(-0.1,0.1,A_shape))
...
def GIB(X,Y):
    Beta = 1
    A = get_init_A(X.shape[0],X.shape[0])
